...
class OffboardControl(Node): 
    ...
    def __init__(self) -> None: # init é a primeira instacia executada quando esse objeto é criado. Antes mesmo da interação com o usuario. Tudo aqui é criado primeiro
       ...
       #subscribers
        ...
        self.takeoff_pointer = [0.0, 0.0, -5.0]
        self.takeoff_pointer2 = [-5.0, 0.0, -5.0]'''
    ...
    def timer_callback(self) -> None:
        ...
        if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD and self.i == 0:
            self.publish_position_setpoint(0.0, 0.0, self.takeoff_pointer)
            #self.publish_velocity_setpoint(-2.0, -2.0, -2.0)
        
        if self.vehicle_local_position.z <= self.takeoff_pointer and self.i == 0:
            self.i = 1
        
        if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD and self.i == 1:
            self.publish_position_setpoint(self.takeoff_pointer2, 0.0, self.takeoff_pointer)

        if self.vehicle_local_position.x <= -5.0 and self.i == 1:
            self.i = 2

        if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD and self.i == 2:
            self.publish_position_setpoint(0.0, -5.0, -5.0)
        
        if self.vehicle_local_position.y <= -5.0 and self.i == 2:
            self.i = 3
        
        if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD and self.i == 3:
            self.publish_position_setpoint(5.0, 0.0, -5.0)
        
        if self.vehicle_local_position.x >= 5.0 and self.i == 3:
            self.i = 4
        
        if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD and self.i == 4:
            self.publish_position_setpoint(0.0, 5.0, -5.0)
        
        if self.vehicle_local_position.y >= 5.0 and self.i == 4:
            self.i = 5
        
        if self.vehicle_status.nav_state == VehicleStatus.NAVIGATION_STATE_OFFBOARD and self.i == 5:
            self.publish_position_setpoint(0.0, 0.0, -7.0)
